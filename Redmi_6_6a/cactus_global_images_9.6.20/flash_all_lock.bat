...
echo "cactus fastboot flash tools"

fastboot %* getvar product 2>&1 | findstr /r /c:"^product: *cactus" || echo Missmatching image and device
fastboot %* getvar product 2>&1 | findstr /r /c:"^product: *cactus" || exit /B 1
...
fastboot %* flash preloader "%~dp0images\preloader_cactus.bin"  || @echo "Flash preloader error" && goto error
