...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        SET _prdName=cactus
        echo HP adalah : Redmi 6A - Cactus
    ) ELSE (
        SET _prdName=cereus
        echo HP adalah : Redmi 6 - Cereus
    )
)
...
REM preloader
"%_FBoot%" flash preloader "%_CSC_Dir%\preloader.bin"  || @echo "Flash preloader error" && goto error
...
REM lk lk.img
"%_FBoot%" flash lk "%_CSC_Dir%\lk.img"  || @echo "Flash lk error" && goto error
...
REM lk2 lk.img
"%_FBoot%" flash lk2 "%_CSC_Dir%\lk.img"  || @echo "Flash lk2 error" && goto error
